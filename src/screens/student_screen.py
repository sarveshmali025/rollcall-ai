...
def student_dashboard():
    ...
    with c2:
        ...
        if toggle_with_action("Logout", type='secondary', key='loginbackbtn', shortcut="control+backspace"):
...
def student_screen():
    ...
    # --- Top row: header/logo top-left, theme toggle + back button top-right ---
    top_left, top_right = st.columns([2.5, 1.5], vertical_alignment='center')
    with top_left:
        header_dashboard()
    with top_right:
        if toggle_with_action("Go back to Home", type='secondary', key='loginbackbtn'):
